...
pub fn write_json_to_file<T>(data: T, path: &str) -> Result<(), Box<dyn std::error::Error>>
where
    T: serde::Serialize,
{
    ...
    let log_json_info = serde_json::to_string_pretty(&data)?;
    file.write_all(log_json_info.as_bytes())?;
