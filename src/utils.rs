...
const LOGGER: &str = "log/application.log";
...
/// # Returns
///
/// * If success : Result in string with date and hour content
/// * If failure : Error string "no_time_avail"
pub fn get_current_datetime() -> String {
    match catch_unwind(AssertUnwindSafe(|| Local::now())) {
        Ok(now) => now.format("%Y-%m-%d %H:%M:%S").to_string(),
        Err(_) => "no_time_avail".to_string(),
    }
}
