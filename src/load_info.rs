...
const LOGGER: &str = "log/load_system_data.json";
...
/// # Returns
///
/// - Vector which contain number of days, hours, and minutes since last boot
...
fn collect_load_data() -> Result<SystemLoad, String> {
    let content = match read_file_content(LOADAVG) {
        ...
        _none => return Err("Reading file error".to_string()),
    };
    ...
    let uptime: (u64, u64, u64) = get_uptime().unwrap_or((0, 0, 0));

    let result: SystemLoad = SystemLoad {
        uptime,
