...
fn collect_ram_data() -> Result<RAMInfo, String> {
    ...
    let result: RAMInfo = RAMInfo {
        ...
        ram_available: format!("{:.2} GB", *values.get("MemAvailable").unwrap_or(&0) as f64 / 1024.0 / 1024.0),
        ram_usage: used_percentage,
        ram_free: format!("{:.2} GB", *values.get("MemFree").unwrap_or(&0) as f64 / 1024.0 / 1024.0),
        mem_buffers: format!("{:.2} MB", *values.get("Buffers").unwrap_or(&0) as f64 / 1024.0),
        mem_cached: format!("{:.2} GB", *values.get("Cached").unwrap_or(&0) as f64 / 1024.0 / 1024.0),
        kernel_stack: format!("{:.2} MB", *values.get("KernelStack").unwrap_or(&0) as f64 / 1024.0),
        mem_locked: format!("{:.2} KB", *values.get("Mlocked").unwrap_or(&0)),
        page_tables: format!("{:.2} GB", *values.get("PageTables").unwrap_or(&0) as f64 / 1024.0 / 1024.0),
...
pub fn get_ram_info() {
    match collect_ram_data() {
        Ok(ram_info) => {
            let ram_json_info: serde_json::Value = json!({ HEADER: ram_info });
