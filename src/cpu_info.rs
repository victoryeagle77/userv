...
/// If resources are not accessible, return 0.0 as value and log the error.
///
...
fn get_cpu_usage() -> Vec<f32> {
    ...
    if cpus.is_empty() {
        error!("[CPU_CORE_USAGE] 'Unable to get CPU core information'");
        return vec![0.0];
    }
    ...
    cpus.iter()
        ...
        .map(|(index, cpu)| {
            ...
            if usage.is_nan() || usage.is_infinite() {
                error!(
                    "[CPU_CORE_USAGE] 'Unable to get valid CPU usage for core {}'",
                    index
                );
                0.0
            } else {
...
/// - An error message if no thermal files or data are found.
...
/// - An error message if no energy consumption file or data are found.
