...
* Linux operating system (with DMI file system available).
* Root permissions for DMI access.
...
|----|-----------|-----|
|`bandwidth_read`|Test bandwidth for reading bytes|MB/s|
|`bandwidth_write`|Test bandwidth for writing bytes|MB/s|
|`ram_available`|Available RAM memory|MB|
|`ram_free`|Free RAM memory|W|
|`ram_power_consumption`||MB|
|`ram_total`|RAM memory usage|MB|
|`ram_types`|Detected hardware device memory|MB|
|`swap_free`|Free SWAP memory|MB|
|`swap_total`|Total SWAP memory|MB|
|`swap_usage`|Total SWAP usage|MB|

For each memory device module, we collect these metrics:

|`kind`|Type of a memory device|MB|
|`id`|Serial number of a memory device|none|
|`voltage`|Configured voltage of a memory device|mV|
|`size`|Size of a memory device|MB|
|`speed`|Size of a memory device|MB|
