...
* `ram_types` : Detected hardware device memory.
...
* `kind` : Type of a memory device.
* `id` : Serial number of a memory device.
* `voltage` : Configured voltage of a memory device in V.
* `size` : Size of a memory device in MB.
