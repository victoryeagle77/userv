...
pub fn mem_data_build(
    ...
    let ram_devices = data_ram_devices.as_ref().filter(|d| !d.is_empty());
    let ram_power_consumption =
        ram_devices.and_then(|devices| mem_estimated_power_consumption(devices, ram_used));
    let ram_devices_cloned = ram_devices.cloned();
...
mod tests {
    ...
    fn test_build_mem_info_with_devices() {
        ...
        let (_ram_info, devices) = mem_data_build(ram_test, ram_device, &sys);
