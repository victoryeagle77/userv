...
impl MemDeviceInfo {
    ...
    pub fn insert_db(
        ...
        if let Some(data) = data {
            for module in data {
                let query_device = db_insert_query(TABLE_NAME[1], &field_descriptor_device())?;
                conn.execute(
                    &query_device,
                    params![
                        timestamp,
...
pub fn get_mem_info() -> Result<(), Box<dyn Error>> {
    ...
    let (data, devices) = mem_data_build(ram_test, ram_device, &sys);
    let timestamp = Utc::now().to_rfc3339_opts(SecondsFormat::Millis, true);
    MemInfo::insert_db(&mut conn, &timestamp, &data)?;
    MemDeviceInfo::insert_db(&mut conn, &timestamp, devices.as_ref())?;
