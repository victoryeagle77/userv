...
/// Collection of collected computing memory based in bytes.
#[derive(Serialize)]
struct RAMInfo {
...
fn collect_ram_data() -> Result<RAMInfo, Box<dyn Error>> {
    ...
    let mem_total = sys.total_memory() / FACTOR;
    let mem_available = sys.available_memory() / FACTOR;
    let mem_free = sys.free_memory() / FACTOR;
