...
fn read_dmi_data() -> HashMap<String, String> {
    MOTHERBOARD_FILES
        ...
        .filter_map(|&path| match read_file_content(path) {
            ...
            None => {
                error!("[{HEADER}] File 'Failed to read file' : {path}");
                None
            }
        })
        .collect()
...
fn collect_motherboard_data() -> MotherboardInfo {
    ...
    for (key, value) in dmi_info.iter() {
        match key.as_str() {
            "board_name" => info.board_name = Some(value.clone()),
            "board_serial" => info.board_serial = Some(value.clone()),
            "board_version" => info.board_version = Some(value.clone()),
            "board_vendor" => info.board_vendor = Some(value.clone()),
            "bios_date" => info.bios_date = Some(value.clone()),
            "bios_release" => info.bios_release = Some(value.clone()),
            "bios_vendor" => info.bios_vendor = Some(value.clone()),
            "bios_version" => info.bios_version = Some(value.clone()),
