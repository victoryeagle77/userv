...
fn estimate_cpu_overhead(
    ...
    // Example coefficients (to be calibrated by real measurements)
    let data_overhead = 0.05 * (received + transmitted); // watts per MB
    let packets_overhead = 0.1 * (packets_received + packets_transmitted); // watts per million packets
