...
/// Types de trafic réseau (pour différenciation énergétique)
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
...
pub fn guess_traffic_type(
    ...
    let total_mb = received_mb + transmitted_mb;
    ...
    if total_mb < 1.0 && total_packets > 0.5 {
        TrafficType::VoIP
    }
    // Beaucoup de données, peu de paquets => Bulk (transfert fichiers)
    else if total_mb > 10.0 && total_packets < 0.1 * total_mb {
        TrafficType::Bulk
    }
    // Débit intermédiaire, nombre de paquets intermédiaire => Video
    else if total_mb > 5.0 && total_packets > 0.2 * total_mb {
        TrafficType::Video
    }
    // Faible trafic => Interactive
    else if total_mb < 2.0 {
