...
fn collect_gpu_data() -> Result<Vec<GpuInfo>, Box<dyn std::error::Error>> {
    ...
    let gpus: u32 = nvml.device_count()?;
    ...
    for index in 0..gpus {
        let device: nvml_wrapper::Device<'_> = nvml.device_by_index(index)?;
...
pub fn get_gpu_info() {
    let data_generator = || -> Result<Value, Box<dyn Error>> {
        let values: Vec<GpuInfo> = collect_gpu_data()?;
